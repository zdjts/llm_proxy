...
export function Sidebar({ mobileOpen, onMobileClose }: Props) {
  ...
  const navigation = (isMobile: boolean) => <div className="flex h-full flex-col bg-white">
    <div className="flex h-[68px] items-center border-b border-surface-200 px-4">
      <div className="flex h-8 w-8 items-center justify-center rounded-md bg-primary-700 font-mono text-sm font-semibold text-white">P</div>
      <div className="ml-3 min-w-0"><div className="truncate text-sm font-semibold text-surface-800">{t.sidebar.brand}</div><div className="font-operational text-[10px] uppercase text-surface-400">{t.sidebar.version}</div></div>
      {isMobile && <button ref={closeRef} type="button" onClick={onMobileClose} className="ml-auto rounded-md p-2 text-surface-500 hover:bg-surface-100" aria-label={t.sidebar.closeNavigation}><X size={18} aria-hidden="true" /></button>}
    </div>
    <nav className="min-h-0 flex-1 overflow-y-auto px-3 py-4" aria-label={t.sidebar.navigation}>{Array.from(sections.entries()).map(([section, items]) => <div key={section} className="mb-5 last:mb-0"><div className="px-2 pb-1.5 font-operational text-[10px] font-medium uppercase text-surface-400">{t.sidebar.sections[sectionLabels[section]]}</div><div className="space-y-0.5">{items.map((item) => <NavLink key={item.to} to={item.to} end={item.to === '/console'} onClick={isMobile ? onMobileClose : undefined} className={({ isActive }) => `flex items-center gap-3 rounded-md px-2.5 py-2 text-sm font-medium transition-colors ${isActive ? 'bg-primary-50 text-primary-800 shadow-sm ring-1 ring-inset ring-primary-200' : 'text-surface-600 hover:bg-surface-100 hover:text-surface-900'}`}><item.icon size={16} className="shrink-0" aria-hidden="true" /><span>{t.sidebar[itemLabels[item.label]]}</span></NavLink>)}</div></div>)}</nav>
  ...
  return <>
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[248px] border-r border-surface-200 bg-white lg:block">{navigation(false)}</aside>
    {mobileOpen && <><button type="button" className="fixed inset-0 z-40 bg-surface-900/35 lg:hidden" onClick={onMobileClose} aria-label={t.sidebar.closeNavigation} /><aside ref={drawerRef} className="fixed inset-y-0 left-0 z-50 w-72 border-r border-surface-200 shadow-xl lg:hidden" aria-modal="true" role="dialog" aria-label={t.sidebar.navigation}>{navigation(true)}</aside></>}
  </>;
}
