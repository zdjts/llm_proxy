  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-surface-200 pb-5">
        <div>
          <h1 className="font-serif text-[26px] font-bold tracking-[0.005em] text-surface-900 sm:text-[32px]">{t.clientKeys.title}</h1>
          <p className="mt-1 text-[14.5px] text-surface-600">{t.clientKeys.subtitle}</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-surface-400">{t.clientKeys.keysCount.replace('{n}', String(data?.total ?? 0))}</span>
          <button onClick={() => { clearNewKey(); setShowAdd(true); }} className="btn-primary flex items-center gap-1.5 text-sm"><Plus size={14} /> {t.clientKeys.addKey}</button>
        </div>
      </div>

      {error && <div className="rounded-[10px] border border-danger-light bg-surface-50 p-3 text-sm text-danger-dark">{error}</div>}

      {showAdd && (
        <div className="glass-card space-y-3 p-5">
          <div className="mb-1 flex items-center gap-2"><Key size={14} className="text-surface-800" /><span className="text-sm font-medium text-surface-700">{t.clientKeys.newKey}</span></div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={closeAdd} className="px-4 py-2 text-xs text-surface-500 transition-colors hover:bg-surface-50 hover:text-surface-900">{t.clientKeys.cancel}</button>
          </div>
        </div>
      )}
      ...
      {rotateKey.hash && (
        <div className="glass-card space-y-3 p-5">
          <div className="mb-1 flex items-center gap-2"><RefreshCw size={14} className="text-surface-800" /><span className="text-sm font-medium text-surface-800">{t.clientKeys.rotateKey}: <span className="font-mono text-xs">{rotateKey.hash}</span></span></div>
          <p className="text-xs text-surface-500">{t.adminUi.secretHint}</p>
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <button onClick={() => rotateMut.mutate()} className="btn-primary text-xs">{t.clientKeys.rotate}</button>
            <button onClick={closeRotate} className="px-4 py-2 text-xs text-surface-500 transition-colors hover:bg-surface-50 hover:text-surface-900">{t.clientKeys.cancel}</button>
          </div>
        </div>
      )}

      {isLoading ? <Skeleton className="h-64 w-full" /> : isError ? <ErrorState action={<Button size="sm" variant="secondary" onClick={() => refetch()}>{t.common.uiRetry}</Button>} /> : data?.keys.length === 0 ? <EmptyState title={t.clientKeys.noKeys} /> : <div className="glass-card overflow-hidden">
                ...
                <tr key={k.key_hash} className="border-b border-surface-50 hover:bg-surface-50/50 transition-colors">
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => setRotateKey({ hash: k.key_hash, newKey: '' })} className="p-2 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-surface-900 transition-colors" title={t.clientKeys.rotate}><RefreshCw size={14} /></button>
                      <button onClick={() => { if (window.confirm(t.adminUi.confirmDelete)) deleteMut.mutate(k.key_hash); }} className="p-2 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-danger-dark transition-colors" title={t.adminUi.delete} aria-label={`${t.adminUi.delete} ${k.key_hash}`}><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
