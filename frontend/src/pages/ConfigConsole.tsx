...
export function ConfigConsolePage() {
  ...
  return <div className="space-y-6">
    <header className="flex border-b border-surface-200 pb-5 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h1 className="text-xl font-semibold text-surface-900 sm:text-2xl">{t.configAdmin.configTitle}</h1><p className="mt-1 text-sm text-surface-500">{t.configAdmin.configSubtitle}</p></div><Button variant="secondary" onClick={() => downloadExport().catch(reportError)}><Download size={15} /> {t.configAdmin.exportConfig}</Button></header>
    {overview.isLoading ? <Skeleton className="h-28 w-full" /> : overview.isError ? <ErrorState action={<Button size="sm" variant="secondary" onClick={() => overview.refetch()}>{t.adminUi.retry}</Button>} /> : <div className="grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-surface-200 bg-surface-200 md:grid-cols-5">{metrics.map(([label, value]) => <Card className="rounded-none border-0 p-4 shadow-none" key={label}><div className="text-xs font-medium text-surface-500">{label}</div><div className="mt-1 text-2xl font-semibold text-surface-900">{value}</div></Card>)}</div>}
    <Card className="space-y-4 rounded-lg p-5"><div className="flex items-center gap-2"><ShieldCheck size={16} className="text-emerald-600" /><h2 className="font-semibold text-surface-900">{t.configAdmin.validateImport}</h2></div><p className="text-xs text-surface-500">{t.configAdmin.configHint}</p><textarea className="min-h-72 w-full rounded-md border border-surface-200 p-3 font-mono text-xs" value={yaml} onChange={(e) => setYaml(e.target.value)} placeholder={t.configAdmin.configPlaceholder} /><div className="flex flex-wrap gap-2"><Button variant="secondary" disabled={busy} onClick={() => run(() => validate.mutate())}>{t.configAdmin.validate}</Button><Button variant="secondary" disabled={busy} onClick={() => run(() => dryRun.mutate())}>{t.configAdmin.dryRun}</Button><Button disabled={busy} onClick={() => run(() => { if (window.confirm(t.configAdmin.confirmCommit)) commit.mutate(); })}>{t.configAdmin.commitImport}</Button><Button variant="secondary" disabled={busy} onClick={() => refresh.mutate()}><RefreshCw size={14} /> {t.configAdmin.refresh}</Button></div>{message && <div role="status" className={`rounded-md p-3 text-sm ${messageKind === 'error' ? 'bg-danger-light text-danger-dark' : 'bg-success-light text-success-dark'}`}>{message}</div>}</Card>
    <Card className="rounded-lg p-5"><h2 className="mb-2 font-semibold text-surface-900">{t.configAdmin.notManaged}</h2><p className="text-sm text-surface-500">{t.configAdmin.notManagedHint}</p></Card>
  </div>;
}
