//! Integration tests for [`AzureProvider`] using mockito (T72 — Fix 6).
//!
//! Pattern identical to `tests/provider_openai.rs`.
...
fn test_provider(server: &ServerGuard) -> AzureProvider {
    let codes: Arc<[u16]> = Arc::from([401, 402, 403, 429]);
    // IMPORTANT: base_url must end with '/' — AzureProvider::chat() strips
    // trailing slash then appends "chat/completions?..."
    AzureProvider::new(
        "azure-test".into(),
        format!("{}/", server.url()),
        "2024-06-01".into(),
...
async fn it_returns_once_on_2xx_non_stream() {
    ...
    server
        ...
        .match_header("api-key", "sk-azure-test")
...
async fn it_returns_stream_on_2xx_stream() {
    ...
    server
        ...
        .with_status(200)
        .with_body(
            "data: {\"id\":\"s1\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"gpt-4o\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"Hi\"},\"finish_reason\":null}]}\n\n\
             data: [DONE]\n\n",
        )
        .create_async()
        .await;
...
async fn it_returns_bad_key_hint_on_401() {
    let mut server = mockito::Server::new_async().await;
    server
        .mock("POST", "/chat/completions?api-version=2024-06-01")
        .with_status(401)
