//! Integration tests for [`BedrockProvider`] using mockito (AUDIT-06).
//!
...
fn test_provider(server: &ServerGuard) -> BedrockProvider {
...
async fn it_returns_once_on_2xx_non_stream() {
    ...
    server
        .mock("POST", "/")
        .match_header("x-api-key", "sk-bedrock-test")
        .match_header("x-aws-region", "us-east-1")
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(
            r#"{"id":"chat-1","object":"chat.completion","created":123,"model":"claude-sonnet-4-20250514","choices":[{"index":0,"message":{"role":"assistant","content":"Hello!"},"finish_reason":"stop"}],"usage":{"prompt_tokens":10,"completion_tokens":5,"total_tokens":15}}"#
        )
        .create_async()
        .await;
...
async fn it_returns_stream_on_2xx_stream() {
    ...
    server
        ...
        .with_status(200)
        .with_body(
            "data: {\"id\":\"s1\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"claude-sonnet-4-20250514\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"Hi\"},\"finish_reason\":null}]}\n\n\
             data: [DONE]\n\n",
        )
        .create_async()
        .await;
