//! Integration tests for [`MistralProvider`] using mockito (AUDIT-06).
//!
...
fn test_provider(server: &ServerGuard) -> MistralProvider {
    let codes: Arc<[u16]> = Arc::from([401, 402, 403, 429]);
    MistralProvider::new("mistral-test".into(), server.url(), codes)
}

fn test_key() -> KeyEntry {
    KeyEntry::api_key("sk-mistral-test", 1)
}
...
async fn it_returns_once_on_2xx_non_stream() {
    ...
    server
        .mock("POST", "/")
        .match_header("authorization", "Bearer sk-mistral-test")
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(
            r#"{"id":"chat-1","object":"chat.completion","created":123,"model":"mistral-large","choices":[{"index":0,"message":{"role":"assistant","content":"Hello!"},"finish_reason":"stop"}],"usage":{"prompt_tokens":10,"completion_tokens":5,"total_tokens":15}}"#
        )
        .create_async()
        .await;
...
async fn it_returns_stream_on_2xx_stream() {
    ...
    server
        ...
        .with_status(200)
        .with_body(
            "data: {\"id\":\"s1\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"mistral-large\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"Hi\"},\"finish_reason\":null}]}\n\n\
             data: [DONE]\n\n",
        )
        .create_async()
        .await;
...
fn it_declares_capabilities() {
    let provider = MistralProvider::new("test".into(), "http://localhost:1/".into(), Arc::from([]));
